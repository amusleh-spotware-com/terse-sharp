...
- An E2E test now locks the `delete_symbol symbolIds=` exclusion for a reference that sits inside another member of the same batch (I661). A temporary probe declares a private helper whose only caller is an unused public member: deleting the helper alone is refused (`'OnlyEntryCalls' still has 1 usages`), and deleting both in one batch lands, helper first. No tool, parameter, default or response format changed. Pinned by `DeleteSymbol_WithSymbolIdsWhereOneIsReferencedOnlyInsideAnother_DeletesBothAsOneEdit`.

- **`terse call` reports where its time went (I655).** After the answer, the one-shot probe writes one `timing loadMs=N callMs=N` line to **stderr**: `loadMs` is the `--workspace` load (0 when it is omitted - an auto-discovered load then lands in `callMs`), `callMs` is the tool invocation alone, including the first compilation a semantic tool triggers. stdout stays the tool's answer byte for byte, and a call refused before the tool runs prints no timing line. A before/after of one tool now compares `callMs` instead of a wall clock the MSBuild load dominates. Pinned by `Call_WithAWorkspace_ReportsLoadAndCallTimeOnStandardErrorAndLeavesTheAnswerUntouched`, `Call_WithoutAWorkspace_ReportsZeroLoadTime`, `Call_RefusedBeforeItRuns_ReportsNoTiming` and `ToString_TruncatesBothPhasesToWholeMilliseconds`.

- **`.terse.json` naming judges a test method by its own `testMethod` pattern, which accepts Method_Scenario_Outcome by default (I654).** A method carrying `[Fact]`, `[Theory]`, `[Test]`, `[TestMethod]`, `[DataTestMethod]` or `[TestCase]` (bare, namespace-qualified or with the `Attribute` suffix) was held to the `method` pattern `^[A-Z][A-Za-z0-9]*$`, so `analyze ids=TERSE107` over this repo's unit tests returned 1 289 records and an end-of-task sweep buried the one actionable finding among ~120. The new naming key `testMethod` defaults to `^[A-Z][A-Za-z0-9]*(_[A-Za-z0-9]+)*$`; a custom `method` pattern no longer applies to test methods - set `testMethod` to hold them to a stricter one. A `const` local is judged by the `constant` pattern like a `const` field, instead of the camelCase `local` one: after the test-method change the 21 records left on this repo's unit tests were all PascalCase `const` locals. Pinned by `Inspect_ForAPascalCaseConstLocal_JudgesItByTheConstantPattern`, `Inspect_ForATestMethodNamedMethodScenarioOutcome_IsNotANamingViolation`, `Inspect_ForAMethodWithUnderscoresAndNoTestAttribute_IsStillANamingViolation`, `Parse_WithATestMethodNamingPattern_ReplacesOnlyTestMethods` and `WriteText_WithAMethodScenarioOutcomeTestName_PassesNamingOnlyWhileItCarriesATestAttribute`.

- **`rerun_failed` keeps a remembered failure until a run passes it (I650).** Every `run_tests` call used to replace the remembered failure list with its own result, so a narrower green `run_tests test=` answered the next `rerun_failed` with `no failing test is remembered` and cost a full re-run of the red suite. A run now removes only the remembered failures its trx reports as passed, adds its own failures when it ran the same target under the same configuration, target framework and properties, and replaces the list only when it failed under another target or scope, or ran in another workspace. A run whose build failed no longer empties the list, and a green unfiltered run of the same target and scope that reported tests replaces it (never a failed build, a `projects=` subset or a `changed=true` narrowing), so a failure whose test was deleted is forgotten (`After_AGreenUnfilteredRunOfTheSameTargetThatNoLongerContainsTheFailure_ForgetsIt`). The refusal's remedy now reads `a remembered failure is forgotten only once a later run passes it`. Pinned by `RerunFailed_AfterANarrowerGreenRun_StillReplaysTheWiderRunsFailures`, `Remember_AGreenRunThatNeverRanTheRememberedFailure_KeepsIt` and `After_ARedRunOfTheSameTargetAndScope_UnionsItsFailuresWithTheOutstandingOnes`; the test that pinned the overwrite is replaced by `Remember_AGreenRunThatPassedTheRememberedFailure_LeavesNothingToRerun`.
...
- **The file watcher no longer writes an absorbed external edit over a newer one (I653).** `MSBuildWorkspace.TryApplyChanges` writes every changed document to disk, and `WorkspaceSync` checked each absorbed path's stamp once, before an async rebase and two lock waits - so a concurrent server over the same tree, a `--read-only` one included, could revert another process's later write with the text it had absorbed. The stamps are now re-checked inside the apply locks, immediately before `TryApplyChanges`, and a moved file is requeued instead of overwritten. The window shrinks to the synchronous apply itself; Roslyn has no public way to absorb without writing. `TerseServerFixture` now fails its collection cleanup when `fixtures/FixtureSolution` holds a `git status` entry the collection did not start with, naming each path. Pinned by `SyncAsync_WhenTheFileMovesBetweenTheAbsorbAndTheApply_KeepsTheNewerTextAndAbsorbsItNext` (observed red with the watcher's predicate reverted), `AdoptAsync_WhenTheFileMovedOnBeforeTheApply_NeverWritesTheOlderText` and `AdoptAsync_WhenTheFileIsStillCurrent_AdoptsTheAbsorbedText`.
- **`replace_symbol` on a TYPE names the members its declaration drops (I651).** A positional record sent as `record Foo(int A);` over a record with a body - a complete declaration, not a header - replaced the body silently, and only the compile gate's `CS1061` rollback showed it. The replace, its `dryRun` and its rollback now carry `WARNING this replace drops N member(s) the type declared: Foo.With(int), ... - a header alone, with no body and no ';', re-heads the type and keeps every member`; a rollback also carries any `attributes dropped` warning it used to lose. A member whose signature changed is not reported as dropped. Pinned by `ReplaceSymbol_OnAPositionalRecordSentWithoutItsBody_NamesTheMembersItDrops`, `ReplaceSymbol_RolledBackBecauseItDroppedMembers_NamesThemInTheRejection`, `ReplaceSymbol_OnAPositionalRecordSentAsAHeaderAlone_KeepsTheBodyAndWarnsNothing` and `Replaced_ANestedPositionalRecordSentEndingInASemicolon_NamesEveryBodyMemberItDrops`.
- **`add_member` re-indents a multi-line declaration as one block before splicing it (I652).** A declaration written at column 0 - the way `get_symbol_source` prints one - was parsed as sent, and the formatter re-indented only its statements, so a nested collection initializer's entries landed level with their `{` and its `},` four columns left of it. Both `typeSymbolId=` and `typeSymbolIds=` now shift the block to the column the type's members sit at, as `replace_symbol` already did; the batch path used the type's own column, which is 0 under a file-scoped namespace. Pinned by `AddMember_WithANestedMultiLineInitializer_KeepsItsLayoutRelativeToTheMember` and `AddMember_WithTypeSymbolIdsAndANestedMultiLineInitializer_KeepsItsLayoutRelativeToTheMember`.
- **`write_text` no longer prescribes a no-op retry when MSBuild's build host drops a write that already landed (I666).** The write path catches the build-host RPC failure itself and compares each target's disk text with the requested content, ignoring only the line ending the write adopted. When every target holds it the answer is `ERROR Transient: <Type>: <message>  landed=<paths>  retry would be a no-op`, with a remedy naming `analyze path=<path>` (or `analyze paths=[...]`) for the diagnostics the dropped call could not report; a `files=` batch that landed only partly names `not landed=<paths>` and says to retry only those. When nothing landed, and on a `dryRun`, the answer is the unchanged `ERROR Transient` retry, so a mismatched byte only ever falls back to the old remedy. Pinned by `GuardedAsync_AWriteTheBuildHostDroppedAfterItLanded_AnswersLandedAndSteersToAnalyzeInsteadOfARetry`, `GuardedAsync_ABatchTheBuildHostDroppedHalfway_NamesWhatLandedAndWhatToRetry` and `GuardedAsync_AWriteTheBuildHostDroppedBeforeItLanded_RethrowsSoTheBoundaryStillPrescribesTheRetry`.
- **A pipe-fed `grep`/`rg` with several `-e`/`--regexp` patterns is a stdin reader (I664).** `gh run view 1 --log-failed | grep -e "error CS" -e "OrderService.cs failed"` was denied as C#/.NET source: the second pattern was taken as a file operand. Each `-e` argument is now skipped, and a positional after an `-e` still counts as a file, so `grep -e x Program.cs` stays denied. The row's own `grep -A 25 "…\[FAIL\]" | cut -c 60- | head -40` was already allowed since I638 and is pinned. Pinned by `Inspect_ForAPipeFedPatternThatNamesDotNetSource_AllowsItBecauseItReadsStdin` and `Inspect_ForATextToolThatReachesDotNetSourceBesideAPipe_StillDeniesIt`.
- **The guard expands a variable assigned earlier in the same command (I665).** `S="<scratchpad>"; wc -l "$S/release-fail.log"` was denied as C#/.NET source because `$S/…` was resolved against the cwd. A plain `NAME=value` statement at the start or after `;`, `&&` or a newline, with no `$`, backtick or nested quote in the value, is substituted into later `$NAME`/`${NAME}` references before the out-of-tree test - for judging only, never for a rewrite. Single-quoted references, a value carrying a space or a shell separator, prefix assignments (`S=x cat $S`), assignments behind `||` or a pipe, and commands with a heredoc are never expanded, and an expansion that changes the command's stage count is judged as introducing source; an unassigned `$VAR` still counts as inside the tree. **Behaviour change:** `F=src/App/OrderService.cs; cat "$F"` was allowed and is now denied, as a whole-command block with no rewrite - only when the expansion introduces a .NET source operand; a variable holding any other path keeps the verdict the unexpanded command had. On Windows a Git-Bash `/tmp/...` value is still judged inside the tree (a pre-existing limit, I676), so the expansion helps a native `C:/...` scratch path there. Pinned by `Inspect_ForAVariableAssignedAPathOutsideTheTreeEarlierInTheCommand_AllowsTheReadThatUsesIt`, `Inspect_ForAVariableThatDoesNotSoundlyExpandOutsideTheTree_StillDeniesTheRead` and `Inspect_ForAVariableAssignedADotNetSourcePath_DeniesTheReadThatUsesItWithoutARewrite`.
- `get_symbol_source` (and every `symbolId` tool) resolves a constructor's metadata name: `Type..ctor` and `Type..cctor` answered `SymbolNotFound`/`AmbiguousSymbol ... matches more than 100 symbols`, because the name was split at the dot inside `.ctor`. They now resolve exactly as `Type.#ctor` does. A **primary constructor** now answers its type's header through the parameter and base list, at the header's own line range, instead of the whole type: `BuildTools.#ctor` returned lines 9-758 and now returns line 9. Pinned by `Parse_ForAMetadataConstructorName_SplitsBeforeTheConstructorsOwnDot`, `GetSymbolSource_WithAMetadataConstructorName_ReturnsTheConstructorTheHashFormReturns` and `GetSymbolSource_OnAPrimaryConstructor_ReturnsTheTypeHeaderThroughItsParameterList`. (I657)
- **`terse uninstall` now removes the guard it installed (I660).** It used to delete only the `mcpServers` entry, leaving the `PreToolUse` guard and the `PostToolBatch` hook in Claude Code's `settings.json` calling a binary that was gone, on every tool call. When claude-code is uninstalled, every hook whose command is `terse guard…` is removed from every event; a matcher left with no hooks is dropped, and so is an event left empty. Every other hook, including a user hook sharing a matcher with the guard, is kept. The command prints `removed guard from <path>`, or `skipped guard (not a JSON object: <path>)` for a settings file it cannot parse, which it leaves untouched; a hook whose `command` is not a string no longer throws while the guard is detected. Pinned by `Unregister_RemovesEveryTerseGuardHook_AndKeepsEveryOtherHookIntact`, `Unregister_WhenAnotherHookHasANonStringCommand_KeepsItAndStillRemovesTheGuard` and `Unregister_ForAnotherClient_LeavesTheGuardInstalled`.
- **`analyze ids=TERSE1xx` no longer answers `NOT_ENABLED` beside the records it just returned (I658).** Policy findings are not analyzer descriptors, so `ids=TERSE107` printed 1 289 `TERSE107` records and `NOT_ENABLED TERSE107` in one answer. An id the active `.terse.json` policy enforces now counts as enabled, the way compiler ids and `TERSE001` already did; a rule set to `off` is still `NOT_ENABLED`. Pinned by `Unsupported_ForAnIdTheActivePolicyEnforces_DoesNotCallItNotEnabled`, `EnforcedIds_ForARuleSetToOff_LeavesOnlyThatIdOut` and `Analyze_WithAnIdTheProjectPolicyEnforces_DoesNotCallItNotEnabled`.
- A steer (`N <tool> calls in a row - …`) or an identical-call note (`repeat #N of this exact <tool> call …`) now starts on its own line. Both are sent as extra content blocks, and a client that concatenates blocks with no separator glued them onto the payload's last line, so `changedLines=41` read as `changedLines=412`. Every trailing note is now merged into the payload's last text block after one line break, so a client that concatenates blocks and one that joins them with a newline both see it on its own line; `NoServerSource_AppendsAContentBlockExceptThroughTrailingNote` fails on any other appender. Pinned by `ASteerAppendedAfterAPayload_StartsOnItsOwnLineWhenTheClientConcatenatesTheBlocks`. (I647)
- The repeat steer no longer claims two calls are "ONE call" when they differ in `dryRun`, `workspace`, `allowErrors` or `verbose`, because one plural call carries only one value of each. A planner subagent's `replace_symbol dryRun=true` made the main thread's next real `replace_symbol` answer `2 replace_symbol calls in a row - these are ONE call: symbolIds=[…]`, naming an id the main thread never sent. The run is still process-wide: over stdio all agents share one MCP session and the request carries no per-agent identity, so two agents sending same-shaped calls still share a run. Pinned by `TwoCallsThatDifferInACallWideArgument_DrawNoSteerClaimingTheyAreOneCall` and `Steer_ForASecondCallWhoseDryRunDiffersFromTheFirst_SaysNothingBecauseOneCallCannotCarryBoth`. (I648)
...
> **Response-format change (MAJOR under this project's rules; on 0.x the MINOR segment carries it).**
> `gate` over its changed-file default and `analyze changed=true` now apply `baseRef=HEAD` on a git
> tree when no `baseRef=` is passed (`baseRef=""` restores every finding), and a narrowed `gate`
> verdict ends `preExisting=N`. A `write_text` that replaces a file with content opens its one-line
> success with `overwrote existing  `, and one keeping under a quarter of that file's content lines is
> refused unless the new `overwrite=true` is passed. An applied edit carrying only `.terse.json` policy
> warnings answers in the condensed form. A `build`/`run_tests` whose output is locked only by another
> process answers `the workspace was NOT unloaded` instead of unloading and retrying. `move_type_to_namespace`
> on a nested type lifts it instead of renaming the file's namespace. New parameters: `run_tests
> detach`/`status`, `gate paths`, `delete_symbol symbolIds`, `write_text overwrite`, `read_text lines`,
> `search_text`/`search_regex paths`; `find_files globs=` now answers beside `root=`.

### Added

- **`run_tests detach=true` answers `run_tests DETACHED id=<id>` at once and runs in the background; `run_tests status="<id>"` answers `run_tests RUNNING id=<id> <n>s` or the finished verdict (I632).** A run past the client's 120 s foreground limit no longer blocks a call or costs a `TaskOutput` poll turn. The detached run holds the solution exactly as a foreground one does, so a second `build`/`run_tests` still answers `RunInFlight`, and a green detached run feeds the `UNCHANGED` memo. Ids live for the process; the 16 newest runs are kept. Covered by `RunTests_Detached_AnswersAnIdAtOnce_AndStatusReturnsTheFinishedVerdict`, `RunTests_StatusOfADetachedRunStillBuilding_AnswersRunningAndItsAge`, `RunTests_StatusOfAnUnknownId_IsRefusedWithARemedy`, `Status_WhilePending_AnswersRunning_AndOnceFinished_AnswersTheVerdict`, `Status_ForARunThatThrew_AnswersTheRenderedErrorRatherThanRunningForever` and `Status_ForAnIdNeverIssued_IsRefusedNamingItWithARemedy`.
- **`gate` takes `paths=`, and the parameter spellings agents carry over from a sibling tool or from `Grep` bind instead of being refused (I644).** A one-week scan counted 20 `InvalidArgument ... unrecognized` refusals, each a re-issued call: `gate paths` 4, `get_type_outline typeName` 4, `load_workspace name` 3, `add_member code` 2, `replace_symbol content` 2, `list_projects contains` 2 and `search_text output_mode`/`-A`/`-C` 2. `gate paths=[...]` gates several files, directories or globs as ONE verdict over their union and refuses by name an entry that matches no document. The rest bind before the unrecognized-parameter check - on the stdio server and in `terse call` alike, at zero schema tokens: `typeName` -> `symbol`, `name` -> `path`, `code`/`content` -> `declaration` (`add_member`, `replace_symbol`), `contains` -> `filter`, and on `search_text`/`search_regex` `-A`/`-B`/`-C` -> `context` and `output_mode=count|files_with_matches|content` -> `countOnly=true|true|false`. An alias passed beside its canonical parameter, an `output_mode` nothing maps and every other unknown name are still refused. `replace_symbol path=` stays refused: that tool resolves no name inside a file. Covered by `Gate_OverSeveralPaths_AnswersOneVerdictOverTheirUnion`, `Gate_WithAPathsEntryMatchingNoDocument_IsRefusedNamingThatEntry`, `ObservedSiblingSpellings_BindToTheirCanonicalParameter_InsteadOfAnsweringInvalidArgument`, `AnAliasBesideItsCanonicalParameter_IsStillRefusedByName`, `Call_WithAnObservedSiblingSpelling_BindsItExactlyAsTheServerWould` and `EveryAlias_NamesAParameterItsToolDeclares_AndIsNotItselfDeclared`.
- **`move_type_to_namespace` on a NESTED type lifts it out of its container to namespace level in the same file (I634)**, instead of renaming the whole file's namespace (which moved the container and left the type nested). It lands `internal` - `public` only when it and its container are both public - and every `Outer.Nested` reference becomes `Nested`, in one compile-gated edit where it used to cost `delete_symbol force allowErrors` + `add_member path=` and a transiently broken tree; the lifted text is dedented by its own column, never reflowed. `targetNamespace` may be omitted or be the file's own namespace; another namespace, a type nested two deep, a generic container, a partial nested type and a name the namespace already declares are refused with a remedy. Covered by `MoveTypeToNamespace_ForANestedPrivateRecord_LiftsItToNamespaceLevelAsInternalAndUnqualifiesItsReferences`, `MoveTypeToNamespace_ForANestedPublicClassOfAPublicType_KeepsItPublic` and `MoveTypeToNamespace_ForANestedTypeAndAnotherNamespace_RefusesAndNamesTheFileNamespace`.
- **`search_text` and `search_regex` take `paths=` (I615)** - up to 10 globs searched as ONE file set, OR-ed by compiled matcher, in the workspace or under `root=`, combining with `glob=` which is taken first. `paths=` is the batch spelling `analyze`, `cleanup`, `format`, `read_text` and `get_file_outline` already take, and it was refused here. Wrapping the globs as one `{g1,g2}` brace glob was refuted: a bare `*.md` would stop matching below the root. Covered by `SearchRegex_WithPaths_SearchesTheUnionOfEveryGlobInOnePass`, `SearchText_WithPathsAndRoot_SearchesTheUnionOfThoseGlobsUnderThatDirectory` and `SearchText_WithABlankPathsEntry_RefusesItByName`.
- **`read_text lines="40-200"` (or `"42"`) (I609)** reads one range as one value, the same read as `startLine=`/`endLine=`, and is refused by name beside them, `tail=` or `ranges=`. To pay for the parameter inside the 1024-token schema cap, the `read_text` description drops its clause about what a verbose whole-file read costs; that behaviour is unchanged. Covered by `ReadText_WithLinesAsOneRange_ReadsExactlyWhatStartLineAndEndLineRead`, `ReadText_WithLinesNamingOneLine_ReadsThatLineOnly`, `ReadText_WithLinesBesideAnotherRangeForm_RefusesNamingLines`, `ReadText_WithLinesBesideRanges_RefusesNamingLines` and `ReadText_WithAMalformedLinesValue_RefusesItByName`.
- **`delete_symbol symbolIds=[...]` deletes several members as ONE compile-gated edit (I608).** Up to 20 ids, across files; a reference that sits inside another member of the same batch no longer counts as a usage, so extracting helpers into a new file and deleting the originals is one call instead of a whole-type `replace_symbol` (~1 900 tokens) or one `delete_symbol` per member. `symbolId=` beside `symbolIds=` is refused, and an id declared in a `.razor` file is refused with a steer to the single `symbolId=` delete, which edits the Razor source. Covered also by `DeleteSymbol_WithSymbolIdsNamingARazorMember_IsRefusedWithTheSingleDeleteRemedy`. Covered by `DeleteSymbol_WithSymbolIds_RemovesEveryMemberAsOneEdit` and `DeleteSymbol_WithSymbolIdsOneOfWhichIsStillReferenced_IsRefusedAndNamesIt`.
- **`terse install --guard` also registers a Claude Code `PostToolBatch` hook** (`terse guard --post-batch`, I643). When a batch held exactly one read-only terse-sharp call, it injects one sentence after the tool results asking the next response to request every independent call at once; any other batch, a writer, a build, or a payload shape it does not recognise injects nothing. An install from an earlier version reads `guard=stale` and is rewritten on the next server start. `terse uninstall` still removes neither hook. Covered by `Decide_ForABatchOfOneTerseReadCall_InjectsTheBatchingNudge`, `Decide_ForAnythingButOneTerseReadCall_InjectsNothing`, `EveryNudgedTool_IsADeclaredTerseTool`, `RunAsync_ForABatchItDoesNotRecognise_WritesNothing`, `InstallGuard_RegistersThePostToolBatchNudgeBesideThePreToolUseGuard_AndKeepsEveryOtherHook` and `AssetsAsync_ForAGuardInstalledBeforeThePostToolBatchHook_ReportsItStale`.

### Changed

- **Response format (I612):** an applied edit whose only extra lines are `.terse.json` policy findings (`WARNING policy  …` or `WARNING policy overridden  …`) now answers in the condensed form - those lines plus one `<path>  changedLines=N` per file - instead of the whole unified diff. `verbose=true` restores the diff; a policy-file notice (unreadable `.terse.json`, unknown rule key) and a `dryRun` still print in full. Measured: `add_member` over 5 files with 19 `TERSE107` warnings answered ~5 500 tokens of diff. Covered by `ApplyAsync_WithActionWarn_AnswersTheWarningAndOneLinePerFileInsteadOfTheDiff`, `ApplyAsync_ForAViolationWithAllowPolicy_AnswersTheOverrideAndOneLinePerFileInsteadOfTheDiff`, `ApplyAsync_WithActionWarnAndVerbose_StillReturnsTheDiff` and `ApplyAsync_WithAMalformedTerseJson_KeepsTheFullResponse`.
- **`write_text` refuses to replace an unrelated file, and says when it overwrote one (I595).** Two files that already existed - a 45-member fixture referenced by four tests, and a 4-test class - were silently replaced in one session by an agent that believed it was creating them; the first cost a 50-failure, 10-minute suite run and 8 repair calls. A single `path=` write now counts how many of the existing file's content lines (trimmed, carrying a letter or digit) survive in the new content and refuses it when fewer than a quarter do, naming the new `overwrite=true` in the remedy; `force=true` does not lift it, a `ref=` restore never needs it, and a `files=[...]` entry is refused the same way (section and row moves stay exempt). A quiet success that replaced a file with content now opens `overwrote existing  <name>  changedLines=N`. The row's literal "shares no line" rule was rejected: every C# file shares its `namespace` line, so it would have missed both incidents. Covered by `WriteText_OverAnExistingFileItKeepsAlmostNothingOf_IsRefusedUnlessOverwriteIsPassed`, `WriteText_ReplacingAnExistingFile_SaysSoInItsOneLineAndCreatingOneDoesNot`, `WriteText_ForcingACSharpFileOverAnUnrelatedOne_IsStillRefusedWithoutOverwrite`, `WriteText_AFilesBatchOverAnUnrelatedExistingFile_IsRefusedUnlessOverwriteIsPassed`, `Measure_RefusesOnlyAWriteThatKeepsUnderAQuarterOfTheContentLines`, `Measure_CountsOnlyLinesCarryingALetterOrDigit` and `Marked_PrefixesOnlyAQuietWriteThatChangedAFileWithContent`; `WriteText_WithRef_RestoresTheFileFromThatRevision` now passes `overwrite=true` for its deliberate damage write.
- **`replace_symbol addTo=` names any type in the workspace, not only the targets' own containers (I614, closes I630).** A batch whose replaced members and new members live in different types - helpers for a type the batch does not replace, or an interface member beside the implementations that satisfy it - was refused with `addTo=X names none of the containing types of these targets`, costing a separate `add_member` plus a `retryWith` replay. A container no target lives in is now resolved as `add_member`'s `typeSymbolId` is and its members join the same compile-gated edit, so a set of declarations that only compiles together is ONE call; a name that resolves to no type answers `addTo=X names no type add= can land in: ...`. Covered by `ReplaceSymbol_AddToNamingATypeNoTargetLivesIn_LandsTheInterfaceMemberBesideItsImplementationsAsOneEdit` and `ReplaceSymbol_AddToNamingNoTypeInTheWorkspace_IsRefusedNamingTheContainer`.
- **A multi-file `usings=` lands each entry only in the files that need it (I611).** `add_member typeSymbolIds=` and `replace_symbol symbolIds=` spanning two or more files keep an entry in a file only when removing it there would add a compile error, and drop it from the others; an entry no file needs still lands everywhere, and a single-file edit is unchanged. Covered by `AddMember_WithTypeSymbolIdsAndUsings_LandsEachUsingOnlyInTheFilesThatNeedIt` and `AddMember_WithTypeSymbolIdsAndAUsingNoFileNeeds_StillLandsItInEveryFile`.
- **`gate` over the changed files defaults to `baseRef=HEAD` and counts what it folds (I613).** A `gate` with no `baseRef=` over 13 changed files returned 90.7 KB, because it listed 369 findings that already existed on those files. On a git tree, the changed-file default now keeps only the findings on lines the working tree changed against HEAD and ends the verdict `preExisting=N`; `baseRef=""` opts back into every finding, `path=`, `paths=` and `solution=true` are unchanged, and a tree git does not track answers as before. Covered by `Gate_OverTheChangedFilesWithNoBaseRef_FoldsWhatTheWorkingTreeDidNotChange_AndBaseRefEmptyOptsOut`.
- **`analyze changed=true` defaults to `baseRef=HEAD`, so the end-of-task sweep reports what the task changed (I607).** `analyze changed=true severity=info` returned 174 records, 172 of them policy findings on members the task never touched, against 2 actionable `CS8019`. With no `baseRef=`, `changed=true` now narrows to the lines the working tree changed against HEAD on a git tree and folds the rest into the existing `N pre-existing finding(s) ... against HEAD` note; `baseRef=""` lists them again. Covered by `Analyze_WithChangedAndNoBaseRef_FoldsWhatTheWorkingTreeDidNotChange_AndBaseRefEmptyOptsOut`.
- **`find_implementations` and `get_symbol usages=true` skip the second whole-solution scan (`FindOverridesAsync`) for a member nothing can override (I605)** - not `virtual`, `abstract` or `override`, `sealed`, or declared in a sealed type or a struct - which is most members; `FindImplementationsAsync` still runs for every member, so interface implementations are unaffected and every answer is unchanged. Covered by `CanBeOverridden_IsTrueOnlyForAMemberSomethingCouldStillOverride`, `FindImplementations_ForANonVirtualMember_AnswersZeroWithTheReason` and the unchanged `FindImplementations_ForAnAbstractMember_NamesTheOverride`.
- **A locked output no longer unloads the workspace when unloading cannot release the lock (I610).** When MSBuild names at least one holder and every named holder is alive, is not this server and is not named `dotnet`, `*BuildHost*` or `*MSBuild*`, `build`/`run_tests` answer `NOTE the output is locked only by processes that are neither this terse server ... the workspace was NOT unloaded and the <operation> was not retried` plus the holder list - no unload, no reload, no second run, and the realized compilations stay warm. A holder that may be this server's own, a gone pid, or no named holder keeps the unload-and-retry. The answer-from-a-pre-build-snapshot half of the row was not built. Covered by `UnloadCannotRelease_WhenEveryNamedHolderIsAnotherLiveProcess_IsTrue`, `UnloadCannotRelease_WhenAHolderMayBeThisServerOrIsGoneOrNoneIsNamed_IsFalse`, `RunTests_WhenTheOutputIsLockedByAnotherProcess_WarnsAndKeepsTheWorkspaceLoadedInsteadOfUnloadingIt` and `ALockedOutput_HeldByAnotherProcess_WithASecondWorkspaceLoaded_LeavesBothLoadedAndRetriesNothing`, which replace the two tests that asserted the retry for a lock held by the test process.
- **Memory pressure is judged against the machine, not a constant** (I631). A non-MRU workspace idle for a minute gives its compilations back once the managed heap passes 60 % of `GC.GetGCMemoryInfo().TotalAvailableMemoryBytes`, never below the previous 2 GB, so a 64 GB box no longer drops warm workspaces at 2 GB. Covered by `Pressure_ScalesToSixtyPercentOfAvailableMemory_WithTwoGigabytesAsTheFloor` and `PressureBytes_OnThisMachine_IsSixtyPercentOfWhatTheGcMayUse`.

### Fixed

- **A piped `grep`/`head` whose pattern names a `.cs` file is a stdin reader again** (I638). `gh run view 1 --log-failed 2>&1 | grep -E "error CS|Failed ToolGuardTests.cs" | head -60` was denied as C#/.NET source because the pattern text was scanned like an operand - `2>&1` and `-E` were not the trigger. A text command fed by a pipe that names no operand, no redirect, no substitution and no recursive or `-f` flag now runs; the same command unpiped, or with a file operand, is still denied, and so are a piped `sed`/`awk` (their script can read a file) and `grep -d recurse`. Covered by `Inspect_ForAPipeFedPatternThatNamesDotNetSource_AllowsItBecauseItReadsStdin` and `Inspect_ForATextToolThatReachesDotNetSourceBesideAPipe_StillDeniesIt`; `Inspect_ForATextReadLaterInACompoundCommand_Denies` now pipes into `grep x Foo.cs`, a real file operand, since a bare pattern no longer counts as one.
- **The guard's out-of-tree carve-out expands `~`, `~/`, `$HOME` and `${HOME}`** (I639). `wc -c ~/notes/hooks.md` and `find ~/.claude/projects -name '*.jsonl'` were denied as C#/.NET source - naming `search_text`, which cannot read a file outside the workspace - while the absolute spelling of the same path was allowed. A home-relative path outside the tree is now judged like its absolute spelling; `~user/`, `$HOMEDIR` and a relative path still count as inside. Covered by `Inspect_ForAHomeRelativePathOutsideTheTree_AllowsItLikeItsAbsoluteSpelling` and `Inspect_ForAPathThatDoesNotExpandToTheUserProfile_StillCountsItAsInsideTheTree`.
- **`replace_symbol` keeps a target's own parameter-list layout when the sent list differs only in whitespace (I633).** A declaration re-typed with its parameters at a different indentation - the same tokens and comments - no longer rewrites the untouched parameter lines; a multi-symbol replace measured 5 collateral hunks, +20 -20 lines of pure whitespace. Any token or comment change still lands as sent. Covered by `ReplaceSymbol_WhoseParameterListDiffersOnlyInLayout_KeepsTheTargetsOwnLayout` and `ReplaceSymbol_WhoseParameterListChangesMoreThanLayout_LandsTheSentList`.
- **`analyze ids=` reads a JSON-array spelling instead of declaring its ids `NOT_ENABLED` (I606).** `ids=["CS8019", "TERSE001"]` answered `NOT_ENABLED ["CS8019", "TERSE001"]` and 0 records, because the ids were split on commas and trimmed of whitespace only, so the brackets and quotes stayed inside each id and matched no diagnostic - the row's descriptor-only diagnosis was wrong, compiler and `TERSE001` ids have been exempt since 0.37.0. Each entry is now trimmed of `[`, `]`, `"` and `'`; `cleanup ids=` shares the parser. Covered by `Analyze_WithIdsWrittenAsAJsonArray_ReadsTheIdsInsteadOfDeclaringThemNotEnabled`.
- **`gate baseRef=` writes only the files the working tree changed against that ref (I635).** `baseRef=` narrowed the findings `gate` reported but not the files its format and cleanup steps wrote, so `gate baseRef=HEAD path="src/**/*.cs"` reformatted six files outside the change set, each costing a restore call. Both write steps now take the same file set the findings are narrowed to, and a scope with no changed file answers `0 files changed - no document in scope differs from the baseRef` instead of writing. Covered by `Gate_WithBaseRefOverAGlob_WritesOnlyTheFilesTheWorkingTreeChanged`.
- **`read_text` reads a file another process holds open for writing (I640).** It read through `File.ReadAllTextAsync`, which opens `FileShare.Read`, so a live test log, build output or transcript answered `ERROR InvalidArgument: IOException` - 17 refusals and 11 shell fallbacks in one week. It now opens `FileShare.ReadWrite | FileShare.Delete`, as `search_text` already did; a handle that shares no read access answers the new `ERROR FileLocked`, whose remedy names `find_files stamps=true` rather than `tail=`, which opens the file the same way. Covered by `ReadText_OnAFileAnotherProcessHoldsOpenForWriting_ReadsItInsteadOfRefusing` and `ReadText_OnAFileHeldWithNoReadSharing_AnswersFileLockedInsteadOfAnArgumentError`.
- **`find_files globs=` answers beside `root=` (I641).** A batch over an outside directory was refused - 22 refusals and 15 retries in one week - although `root=` already answered one glob. The batch now walks that directory once and answers every glob under its own header, tagged `outside-workspace`. Covered by `FindFiles_WithGlobsAndRoot_AnswersEveryGlobUnderItsOwnHeaderOverThatDirectory` and `FindFiles_WithGlobsAndARelativeRoot_RefusesTheRootOnceForTheWholeBatch`, which replace the refusal test.
- **A run of `write_text delete=true` or `ref=` calls no longer draws the `files=[...]` steer (I616).** A `files=` entry carries only `path`, `content` and `force`, so three `obj/` deletes and six `ref=HEAD` restores were each told to batch into a shape that cannot express them. `delete`, `recursive` and `ref` now reset the run the way `startLine`/`endLine`/`tail`/`section` do on `read_text`; a `false` or `null` value does not. Covered by `ARunOfWriteTextDeletesOrRestores_DrawsNoFilesSteerItCannotExpress` and `Unbatchable_ForAWriteTheFilesEntriesCannotExpress_SuppressesTheRunSteer`.
- **`move_type_to_namespace` wrote `namespace X ;`** on a file-scoped namespace, because the new name took a hard-coded trailing space; it now keeps the old name's own trivia. Covered by `MoveTypeToNamespace_RenamingANamespace_WritesNoSpaceBeforeTheNewName`.
...
- `TERSE_UNCHANGED_TRACE=1` appends a `memo:` line to every `build`/`run_tests` response that did not replay, naming the memo decision - `first run of this key`, `stamp moved: remembered '…' current '…'`, `stamp unavailable: <workspace> watch=<state> gaps=<n>`, `not remembered - the verdict is not a single line` - so a memo that never primes diagnoses itself in one run (I532).
- `edit_text` `row=`/`rows=` moves now accept `edits=` entries naming OTHER files, so closing a backlog row and writing its changelog entry is one call instead of two. An entry with no path of its own, or one targeting a file the row move itself rewrites, is refused before anything is written (I533).
...
- `package_add` on a centrally-managed solution wrote the `<PackageVersion>` element into the first `<PropertyGroup>` of `Directory.Packages.props`, breaking central package management for every project (`NU1015` on the next build). It now lands in the `<ItemGroup>` that already holds a `<PackageVersion>`, or a new `<ItemGroup>` when none does - and an `<ItemGroup>` carrying a `Condition` is never chosen, so a per-TFM pinning group cannot swallow an unconditional version (I531).
- The `build`/`run_tests` `UNCHANGED` memo never primed when the first call raced the asynchronous `--workspace` preload: its stamp was captured over zero loaded workspaces, so the repeat's stamp never matched and every repeat re-ran. The stamp is now computed after the preload settles (I532).
- A leading redirect no longer defeats the guard: `> out.log dotnet build X` tokenized with head `>` and was allowed whole. `Tokenized` now skips a prefix redirect operator and its target, so the segment classifies by its real command word and a replaced command behind one is denied - `>f`, `>>`, `2>`, `10>`, `&>`, `<`, `<<` and `<<<` shapes included (I534).
- The `build`/`run_tests` `UNCHANGED` memo could replay a green verdict against changed disk: an external edit the watcher had delivered but not yet absorbed left the generation counters unmoved, and the replay path never drains, so the stale answer repeated until some other call synced. The stamp is now computed after draining every loaded workspace's pending queue, and the `changed_files` listing memo drains the same way - its git-tool path never syncs, so a delivered-but-unabsorbed event used to hold `pending>0` and silently disable that memo for as long as no semantic call ran (I535).

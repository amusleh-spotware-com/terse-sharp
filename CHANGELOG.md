...
- The `build`/`run_tests` `UNCHANGED` memo could replay a green verdict against changed disk: an external edit the watcher had delivered but not yet absorbed left the generation counters unmoved, and the replay path never drains, so the stale answer repeated until some other call synced. The stamp is now computed after draining every loaded workspace's pending queue (I535).

...
- **`rerun_failed` keeps a remembered failure until a run passes it (I650).** Every `run_tests` call used to replace the remembered failure list with its own result, so a narrower green `run_tests test=` answered the next `rerun_failed` with `no failing test is remembered` and cost a full re-run of the red suite. A run now removes only the remembered failures its trx reports as passed, adds its own failures when it ran the same target under the same configuration, target framework and properties, and replaces the list only when it failed under another target or scope, or ran in another workspace. A run whose build failed no longer empties the list. The refusal's remedy now reads `a remembered failure is forgotten only once a later run passes it`. Pinned by `RerunFailed_AfterANarrowerGreenRun_StillReplaysTheWiderRunsFailures`, `Remember_AGreenRunThatNeverRanTheRememberedFailure_KeepsIt` and `After_ARedRunOfTheSameTargetAndScope_UnionsItsFailuresWithTheOutstandingOnes`; the test that pinned the overwrite is replaced by `Remember_AGreenRunThatPassedTheRememberedFailure_LeavesNothingToRerun`.

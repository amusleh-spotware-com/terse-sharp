...
public static class ResxValidation
{
    ...
    public static Result<string> Validate(
        ...
        int maxResults)
    {
        var families = Scope(workspace, path);
        ...
        var tokens = unused ? ResxUsageService.Tokens(workspace.Root) : [];

        var findings = families
            .Value!
            .Where(family => family.Kind is not ResxKind.WinForms)
            .SelectMany(family => Findings(family, selected, tokens, unused))
    ...
    private static Result<IReadOnlyList<ResxFamily>> Scope(LoadedWorkspace workspace, string? path)
    {
        if (path is not { Length: > 0 })
            return Result.Ok<IReadOnlyList<ResxFamily>>(ResxIndex.Build(workspace.Root).Families);
    ...
    private static IEnumerable<ResxFinding> Findings(
        ResxFamily family,
        IReadOnlyList<string> selected,
        HashSet<string> tokens,
        bool includeUnused)
    {
        var neutral = ResxIndex.Read(family.Neutral.Path);
        ...
        [
            .. Duplicates(family),
            .. Whitespace(family),
            .. Cultures(family, document),
            .. Designer(family, document),
            .. Unsorted(family, selected),
            .. includeUnused ? Unused(tokens, family, document) : [],
        ];
    }

    private static IEnumerable<ResxFinding> Duplicates(ResxFamily family) => family
        .Files
        .SelectMany(file => ResxIndex
            .Entries(file)
    ...
    private static IEnumerable<ResxFinding> Whitespace(ResxFamily family) => family
        .Files
        .SelectMany(file => ResxIndex
            .Translatable(file)
    ...
    private static IEnumerable<ResxFinding> Cultures(ResxFamily family, ResxDocument neutral) => family
        .Cultures
        .SelectMany(file => Culture(file, neutral));

    private static IEnumerable<ResxFinding> Culture(ResxFile file, ResxDocument neutral)
    {
        var entries = ResxIndex.Translatable(file);
    ...
    private static IEnumerable<ResxFinding> Unsorted(ResxFamily family, IReadOnlyList<string> selected)
    {
        ...
        return family
            .Files
            .Where(file => ResxIndex.Read(file.Path) is { IsOk: true, Value.IsSorted: false })

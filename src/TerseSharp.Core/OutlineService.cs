...
public static class OutlineService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        return Result.Ok(Render("get_file_outline", path, Declarations(root), model));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return model is null || node is not MemberDeclarationSyntax declaration || !IsTypeDeclaration(declaration)
            ...
            : Result.Ok(Render("get_type_outline", symbol.Name, [declaration], model));
    }
        ...
        SemanticModel model)
    {
        ...
        foreach (var declaration in declarations)
            AppendType(response, declaration, model);

        return response.ToString();
    }

    private static void AppendType(ResponseBuilder response, MemberDeclarationSyntax declaration, SemanticModel model)
    {

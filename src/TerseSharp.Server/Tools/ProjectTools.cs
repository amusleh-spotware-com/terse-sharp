...
public sealed class ProjectTools(ToolContext context)
{
    ...
    public string ProjectProperties(
        ...
        context.WithWorkspace(workspace, project, loaded =>
            NavigationTools.Unwrap(ProjectFile.GetProperties(Resolve(loaded, project), name)));
    ...
    public string PackageList(
        ...
        [Description("Optional workspace path or worktree name.")] string? workspace = null) =>
        context.WithWorkspace(workspace, project, loaded =>
            NavigationTools.Unwrap(ProjectFile.ListPackages(Resolve(loaded, project))));
    ...
    public string PackageAdd(
        ...
        [Description("Return the diff without writing. Default false.")] bool dryRun = false,
        [Description("Optional workspace path or worktree name.")] string? workspace = null) =>
        Guarded(workspace, project, loaded => ProjectFile.AddPackage(Resolve(loaded, project), package, version, dryRun));

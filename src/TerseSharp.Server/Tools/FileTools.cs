...
(glob ?? pattern ?? query) is { Length: > 0 } matched
    ...
    [Description("Literal text search across the workspace, or across any absolute directory with root=. queries=[...] searches up to 10 literals in ONE pass over the same file set, tagging every record q1..qN by the position of the literal in queries=, which replaces one call per literal and the shell grep alternation that cannot say which alternative matched. A line matching several of them is ONE record carrying all of their tags, comma-separated in query order (q1,q3). Keep the entries line-local: a literal containing a newline can match across a line break and then consumes the lines its match spanned, so the other entries' hits on those lines are not reported. Also the counting tool: the count line is how many matching LINES exist, at most one per line, and a zero result proves absence in the files it searched - bin, obj, .git, .claude, .vs, .idea, artifacts, TestResults, node_modules and directory symlinks are skipped. context=N adds the surrounding lines so a hit needs no follow-up read, matchesOnly=true prints the matched span instead of the whole line the way grep -o does, unique=true collapses identical matching lines to one record with x<count>, and exclude= drops the paths a glob= cannot leave out. Results are tagged HEURISTIC: for a type or member name use search_symbols or find_usages instead.")]
    ...
    [Description("Several literals answered in one pass over the same file set, at most 10. Every record is tagged q1..qN by the position of its literal here, so one call answers where are these N things and no legend is echoed back. A line matching several literals is ONE record tagged with all of them, comma-separated in query order. Keep the entries line-local - a literal containing a newline consumes the lines its match spans, hiding the other entries' hits on them. Combines with query, which is taken first; more than 10 entries is refused rather than truncated.")] string?[]? queries = null,
    CancellationToken cancellationToken = default) =>
    ...
    [Description("Glob of paths to drop after glob= has selected them, e.g. .research/** or **/*.generated.cs.")] string? exclude = null,
    [Description("Print the matched span instead of the whole line, the way grep -o does, and compose it with unique=true to answer which distinct values of this shape exist. A match that is only whitespace still prints its line, so no record is ever empty. Default false.")] bool matchesOnly = false,
    [Description("Several expressions answered in one pass over the same file set, at most 10. Every record is tagged q1..qN by the position of its expression here, so the caller can tell which expression produced which record - which one alternation cannot. A line matching several expressions is ONE record tagged with all of them, comma-separated in query order. Keep the expressions line-local - one that spans a line break consumes the lines it matched, hiding the other expressions' hits on them. Combines with query, which is taken first; more than 10 entries is refused rather than truncated.")] string?[]? queries = null,
    CancellationToken cancellationToken = default) =>

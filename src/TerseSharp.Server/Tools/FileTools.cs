...
public sealed class FileTools(ToolContext context)
{
    ...
    public Task<string> ReadText(
                ...
                [Description("End the answer with the file's byte length as bytes=N, on every shape read_text returns and once per paths= entry. Under ref= it is the BLOB's size at that revision, from git cat-file -s rather than the decoded text, so a CRLF checkout cannot skew it; a blob that cannot be sized answers bytes=UNRESOLVED. find_files stamps=true answers the same number without reading the file. Default false.")] bool bytes = false,
                [Description("End the answer with the whole file's token cost as tokens=N, whatever range was read - the count the shipped-doc budgets assert. Default false.")] bool tokens = false,
                ...
                [Description("Markdown only: comma-separated column headers to project every markdown table row down to, e.g. 'Finding,Tool'. Answers which rows a checked-in table holds without returning the whole table. Composes with section=, which scopes the projection to that section's tables and is named in a refusal so the columns it scanned are never mistaken for the file's, and with maxLines= and maxChars=, which bound the rows: a projection that runs out of its CHARACTER budget truncates and names search_regex matchesOnly=true unique=true as the one bounded call that answers the same question, instead of spilling a whole large table; a column no table under the read declares is refused naming it, and headings=true, startLine=, endLine= and tail= beside it are refused rather than silently ignored, because a projection is addressed by table rather than by line.")] string? columns = null,
...
&& (startLine, endLine, tail, maxLines, maxChars) is ( <= 0, <= 0, <= 0, <= 0, <= 0);
    ...
    public Task<string> WriteText(
        ...
        [Description("Allow writing or deleting a .cs file - every .cs entry of a files= batch included - and a single write outside every workspace root. Still compile-gated when a project compiles it.")] bool force = false,
    ...
    public Task<string> SearchText(
        ...
        [Description("Name the C# declaration each hit sits in - Type.Member, from syntax - before the matched line, so the record is an id get_symbol_source takes. .cs only; refused beside countOnly=. Default false.")] bool containers = false,

...
| **the shared fixture is left as the collection found it** | `TerseServerFixture.DisposeAsync` | `git status --porcelain --untracked-files=all -- fixtures/FixtureSolution`, diffed against the same status captured in `InitializeAsync`, after up to 20 x 500 ms of settling |

...
| `tail -n 200` on a log | `read_text(path, tail=200)` | the last N lines; a clipped read ends with `next: startLine=…` so the follow-up never overlaps or gaps, and `maxChars=` bounds a file whose lines are too long for `maxLines` — a budget that runs out **inside** a line says `line N was cut mid-way` rather than offering a `startLine` that cannot resume there |

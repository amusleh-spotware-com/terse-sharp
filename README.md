...
| **Edit** — replaces `Edit` on a `.cs` | `replace_symbol_body` · `replace_symbol` (`addBefore=` / `addAfter=` / `addPosition=` to place the helpers `add=` brings) · `add_member` (`before=` / `after=` / `position=` to place it, not append it) · `delete_symbol` · `rename_symbol` |
